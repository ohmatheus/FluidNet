...
from training.training_plots import plot_loss_components, plot_metrics_grid, plot_training_history
...
class Trainer:
    ...
    def _best_epoch(self) -> int:
        if self.early_stopping is not None and self.early_stopping.best_epoch >= 0:
            return self.early_stopping.best_epoch + 1
        return self.history["val_total"].index(min(self.history["val_total"])) + 1
    ...
    def train(self) -> None:
        ...
        try:
            log_artifact_flat(plot_training_history(self.history, best_epoch), "training_loss_and_lr.png", dpi=144)
            print("Training history plot saved to MLflow artifacts")
        except Exception as e:
            print(f"Warning: Failed to generate/save training history plot - {e}")

        try:
            log_artifact_flat(plot_metrics_grid(self.history), "validation_metrics_grid.png", dpi=90)
            print("Metrics grid plot saved to MLflow artifacts")
        except Exception as e:
            print(f"Warning: Failed to generate/save metrics grid plot - {e}")

        try:
            log_artifact_flat(plot_loss_components(self.history, best_epoch), "loss_components.png", dpi=90)
