...
def save_export_metadata(checkpoint_path: Path, base_onnx_path: Path, variants_info: dict[str, dict]) -> None:
    """
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force re-export even if ONNX file is up-to-date (ignores metadata)"
    )
...
def export_checkpoint(
    display_name: str, checkpoint_path: Path, onnx_path: Path, device: str, input_shape: tuple[int, int, int, int]
) -> bool:
